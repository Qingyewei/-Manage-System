let components = {
    StudyJavaScriptVue:()=>import('./index.vue'),
    JavaScriptdragAndDrop:()=>import('./JavaScript/JavaScriptdragAndDrop.vue'),
    VueShuffling:()=>import('./Vue/shuffling.vue'),
    VueShuffling2:()=>import('./Vue/shuffling2.vue'),
    Addanddelete:()=>import('./Vue/Addanddelete.vue'),
    Addanddelete2:()=>import('./Vue/Addanddelete2.vue'),
    Addanddelete3:()=>import('./Vue/Addanddelete3.vue'),
    search:()=>import('./Vue/search.vue'),
    LibraryManagement:()=>import('./case/LibraryManagement.vue'),
    LibraryManagement2:()=>import('./case/LibraryManagement2.vue'),
}
export default components